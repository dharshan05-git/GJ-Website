...
# Explicit mappings of product name to 4 images
EXPLICIT_MAPPING = {
    # ------------------ SETS (4) ------------------
    ("Sets", "Blue Stone Set"): [
...
# Conversion and catalog construction
products_list = []
total_converted = 0
...
for idx, p in enumerate(pdf_prods):
    ...
    folder_prod_name = p["name"] # might be "Product 2" in PDF or "Sleek Gemstone Ring" in folder
    if folder_prod_name == "Product 2":
        folder_prod_name = "Sleek Gemstone Ring"
    
    source_files = EXPLICIT_MAPPING.get((folder_cat, folder_prod_name))
    if not source_files:
    ...
    for img_idx, sf in enumerate(source_files):
        ...
        with Image.open(src_path) as im:
            ...
            # If image is larger than 1600 in any dimension, resize nicely
    ...
    price = p["price"]
    # Calculate attractive MRP strike-through price
    orig_price = int(round((price * 1.32) / 50) * 50) - 1
    if orig_price <= price:
        orig_price = price + 600
    
    # Clean PDF description
    clean_desc = p["description"]
    clean_desc = clean_desc.replace("-", "").replace("", "–").replace("  ", " ").strip()
    if not clean_desc or len(clean_desc) < 10:
        clean_desc = f"{prod_name} – an exquisite piece crafted in precious gold finish, featuring handcrafted brilliant stone accents and signature Gevariya Jewels craftsmanship."
    else:
        # Add rich closing sentence
        clean_desc = f"{clean_desc} Handcrafted to perfection with signature Gevariya Jewels fine jewelry detailing."
...
print(f"Successfully converted {total_converted} images into WebP format across {len(products_list)} products.")

# Now let's generate the JavaScript file src/data/products.js
...
print("Updated src/data/products.js successfully with all 56 products!")
