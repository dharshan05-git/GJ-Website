 ...
 *  • RESEND_API_KEY set       → Resend API delivery (modern, reliable transactional email)
 *  • SMTP_HOST/USER/PASS set  → SMTP delivery (Brevo, SES, Zoho, Gmail app password…)
...
const buildTransport = async () => {
  ...
  if (!env.isProd) {
    ...
    console.log('📧 No Resend/SMTP configured — using Ethereal test inbox (preview links appear in the logs).');
    return transporter;
  }
...
export const sendMail = async ({ to, subject, html, text, type = 'OTHER', relatedOrder = '' }) => {
  ...
  const plainText = text || html.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 2000);
  ...
  if (resend) {
    try {
      ...
      console.log(`📧 Resend email sent (${type}) to ${recipientList.join(', ')} [ID: ${data?.id}]`);
      return await EmailLog.create({
        to: recipientList.join(', '),
        subject,
        type,
...
export const getTransportStatus = async () => {
  ...
  return {
    mode: env.mail.resendApiKey ? 'RESEND' : env.mail.configured ? 'SMTP' : env.isProd ? 'DISABLED' : 'ETHEREAL_PREVIEW',
    host: env.mail.resendApiKey ? 'api.resend.com' : env.mail.host || null,
    from: env.mail.from,
    sentToday,
    failedToday,
