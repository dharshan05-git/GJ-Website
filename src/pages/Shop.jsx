...
export const Shop = () => {
  ...
  // Reset default filter when main category tab changes
  useEffect(() => {
    setGenderFilter('ALL');
  }, [selectedCategory]);

  const MAIN_CATS = ['ALL', 'RINGS', 'NECKLACES', 'EARRINGS', 'STUDS', 'BRACELETS', 'SETS'];
      ...
      <div className="max-w-[1320px] mx-auto px-4 sm:px-8 py-8 sm:py-12">
        ...
        {selectedCategory === 'EARRINGS' && (
          <div>
            ...
            {renderProductGrid(PRODUCTS.filter(p => p.category === 'EARRINGS'))}
          </div>
        )}

        {/* 2.5 STUDS CATEGORY */}
        {selectedCategory === 'STUDS' && (
          <div className="space-y-10 sm:space-y-12">
            <div>
              <div className="mb-4 sm:mb-6 pb-2 border-b border-[#D8CFC3]">
                <h2 className="font-serif text-lg sm:text-2xl text-[#2E2B2B] uppercase tracking-wide font-normal">
        ...
        {selectedCategory !== 'RINGS' && selectedCategory !== 'EARRINGS' && selectedCategory !== 'STUDS' && selectedCategory !== 'BRACELETS' && (
          <div>
            {renderProductGrid(
