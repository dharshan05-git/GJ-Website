...
import {
  ...
  Settings as SettingsIcon, LogOut, Menu, X, ExternalLink, ShieldAlert,
} from 'lucide-react';
...
/**
 * The Superadmin area, mounted at /admin. It deliberately sits outside the
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (user.role !== 'superadmin') {
        api.logout();
        setError('Only Superadmin accounts can access this panel.');
        return;
      }
      onSignedIn(user);
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-[#F5F1EA] flex items-center justify-center px-4 text-center">
        <div className="max-w-sm bg-white border border-[#E3D9CE] rounded-2xl p-8">
          ...
          <h1 className="font-serif text-xl text-[#2E2B2B] uppercase tracking-wide">Superadmin access only</h1>
          <p className="text-xs text-[#5C4038] mt-2 leading-relaxed">
            You are signed in as {user.email}. Only the Superadmin account can access this panel.
          </p>
          <Button
            variant="secondary"
            className="mt-5"
    ...
    <div className="min-h-screen bg-[#F5F1EA] flex flex-col lg:flex-row">
      ...
      <div className="lg:hidden flex items-center justify-between bg-white border-b border-[#E3D9CE] px-4 py-3 sticky top-0 z-30">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#7B3F42]">Super Admin</span>
        </div>
        <button
