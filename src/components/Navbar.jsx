...
import { Home, User, Search, ShoppingBag, Menu, X, ChevronDown, ArrowRight, ShieldCheck, Truck, RefreshCw, Sparkles } from 'lucide-react';
...
export const Navbar = () => {
  ...
  // Close browse drawer when navigating — do NOT lock body scroll
  // (locking body scroll prevents the drawer itself from scrolling on mobile)
  useEffect(() => {
    return () => {
  ...
  const shopCategories = [
    ...
    { id: 'EARRINGS',  name: 'Earrings',        tag: 'Drops, Hoops & Chandelier' },
    { id: 'STUDS',     name: 'Studs',           tag: 'Solitaires & Micro-Pave' },
  ...
  const collections = [
    ...
    { id: 'STUDS',     name: 'Solitaire Studs',   tag: 'Unisex Diamond & Gem Studs' },
    ...
    <>
      ...
      >
        <div className="max-w-[1320px] mx-auto px-4 sm:px-8 flex items-center justify-between h-[68px] sm:h-[95px]">
          ...
          {/* ── RIGHT: Icons (Desktop & Mobile: Search and Bag only) ── */}
        ...
        {/* ── Mobile Menu Modal / Drawer (Triggered by 'Browse' button in Bottom Nav) ── */}
        {mobileOpen && (
          <div className="lg:hidden fixed inset-x-0 top-[68px] bottom-[56px] z-[39] flex flex-col">
            <div className="bg-[#FAF7F2] border-b border-[#D8CFC3] p-5 space-y-4 flex-1 overflow-y-scroll overscroll-contain shadow-2xl animate-fadeIn">
              
              {/* Drawer Title & Close Button */}
              <div className="flex items-center justify-between pb-2 border-b border-[#D8CFC3]/80">
                <span className="font-serif text-sm font-semibold tracking-wider text-[#2E2B2B] uppercase">
                  EXPLORE BOUTIQUE
                </span>
                <button
                  onClick={() => setMobileOpen(false)}
                  className="text-[#7B3F42] p-1 rounded-full hover:bg-[#E8D5CE]/50 active:scale-95"
                ...
                <div className="grid grid-cols-2 gap-2">
                  {shopCategories.map(cat => (
                    <button
                      ...
                      className="text-left text-[11px] font-medium py-2.5 px-3 bg-white border border-[#D8CFC3] text-[#2E2B2B] hover:border-[#7B3F42] hover:text-[#7B3F42] transition-all rounded-xl flex items-center justify-between shadow-xs"
                    >
                      <span>{cat.name}</span>
      ...
      {/* ── MOBILE BOTTOM NAVIGATION BAR (Matching User Reference Image 1) ── */}
      <nav
        aria-label="Mobile Navigation"
        className="fixed bottom-0 inset-x-0 z-40 bg-white/95 backdrop-blur-md border-t border-[#D8CFC3]/90 shadow-[0_-4px_25px_rgba(0,0,0,0.07)] rounded-t-2xl px-3 py-2 flex items-center justify-around sm:hidden select-none"
      >
        {/* 1. Home */}
        <button
          onClick={() => { go('home'); setMobileOpen(false); }}
          className={`flex flex-col items-center justify-center min-w-[56px] py-0.5 transition-all cursor-pointer ${
            activePage === 'home' && !mobileOpen ? 'text-[#7B3F42]' : 'text-[#5C4038] hover:text-[#7B3F42]'
          }`}
        >
          <Home size={19} strokeWidth={activePage === 'home' && !mobileOpen ? 2.2 : 1.7} />
          <span className={`text-[10px] tracking-wide mt-0.5 ${activePage === 'home' && !mobileOpen ? 'font-bold' : 'font-medium'}`}>
            Home
          </span>
        </button>
        ...
        {/* 3. Customise */}
        <button
          onClick={() => { go('customise'); setMobileOpen(false); }}
          className={`flex flex-col items-center justify-center min-w-[56px] py-0.5 transition-all cursor-pointer ${
            activePage === 'customise' && !mobileOpen ? 'text-[#7B3F42]' : 'text-[#5C4038] hover:text-[#7B3F42]'
          }`}
        >
          <Sparkles size={19} strokeWidth={activePage === 'customise' && !mobileOpen ? 2.2 : 1.7} />
          <span className={`text-[10px] tracking-wide mt-0.5 ${activePage === 'customise' && !mobileOpen ? 'font-bold' : 'font-medium'}`}>
            Customise
          </span>
        </button>
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center min-w-[56px] py-0.5 transition-all cursor-pointer ${
            mobileOpen || activePage === 'shop' ? 'text-[#7B3F42]' : 'text-[#5C4038] hover:text-[#7B3F42]'
          }`}
        >
          {mobileOpen ? (
            <X size={19} strokeWidth={2.2} />
          ) : (
            <Menu size={19} strokeWidth={activePage === 'shop' ? 2.2 : 1.7} />
          )}
          <span className={`text-[10px] tracking-wide mt-0.5 ${mobileOpen || activePage === 'shop' ? 'font-bold' : 'font-medium'}`}>
            Browse
          </span>
        </button>
      </nav>
